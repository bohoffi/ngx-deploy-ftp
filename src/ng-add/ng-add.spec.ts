...
describe("ng add ngx-deploy-ftp", () => {
  ...
  it("fails with a missing tree", async () => {
    await expect(
      testRunner.runSchematicAsync("ng-add", {}, Tree.empty()).toPromise()
    ).rejects.toThrow();
  });
  ...
  it("fails with a non-existing project", async () => {
    await expect(
      testRunner
        .runSchematicAsync("ng-add", corruptAddOptions, Tree.empty())
        .toPromise()
    ).rejects.toThrow();
  });
  ...
  it("adds ngx-deploy-ftp to the default project", async () => {
    ...
    appTree = await testRunner
      .runSchematicAsync("ng-add", emptyAddOptions, appTree)
      .toPromise();
    const angularJson = JSON.parse(appTree.readContent("/angular.json"));

    expect(
      angularJson.projects[appOptions.name].architect.deploy
    ).toBeDefined();
    expect(angularJson.projects[appOptions.name].architect.deploy.builder).toBe(
      "ngx-deploy-ftp:deploy"
    );
  });
  ...
  it("adds ngx-deploy-ftp to an existing project", async () => {
    let appTree = await initAngularProject();
    appTree = await testRunner
      .runSchematicAsync("ng-add", addOptions, appTree)
      .toPromise();
    const angularJson = JSON.parse(appTree.readContent("/angular.json"));
