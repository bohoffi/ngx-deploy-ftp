...
describe("ngx-deploy-ftp ftp-engine", () => {
  it("should pass option verification", () => {
    const spy = jest.spyOn(engine, "verifyRequiredOptions");
  ...
  it("should fail on missing host options", () => {
    expect(() => engine.verifyRequiredOptions(missingHostOptions)).toThrowError(
      "Cannot deploy the application without a host"
    );
  });

  it("should fail on missing username options", () => {
    expect(() =>
      engine.verifyRequiredOptions(missingUsernameOptions)
    ).toThrowError("Cannot deploy the application without a username");
  });

  it("should fail on missing password options", () => {
    expect(() =>
      engine.verifyRequiredOptions(missingPasswordOptions)
    ).toThrowError("Cannot deploy the application without a password");
  });
});
