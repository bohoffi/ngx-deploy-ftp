...
const mockFtpEngine: {
  ...
  run: (_: string, __: DeployOptions, __2: logging.LoggerApi) =>
    Promise.resolve(),
};
...
describe("ngx-deploy-ftp ftp-deploy", () => {
  ...
  it("should build the project", async () => {
    const spy = jest.spyOn(builderContextMock, "scheduleTarget");
  ...
  it("should invoke the engine", async () => {
    const spy = jest.spyOn(mockFtpEngine, "run");
    await ftpDeploy(mockFtpEngine, builderContextMock, deployOptions);

    expect(spy).toHaveBeenCalledWith(
      targetOutputPath,
      deployOptions,
      builderContextMock.logger
    );
  });
  ...
  describe("ngx-deploy-ftp ftp-deploy error handling", () => {
    it("throws if there is no target", async () => {
      ...
      await expect(
        ftpDeploy(mockFtpEngine, builderContextMock, deployOptions)
      ).rejects.toThrowError("❌ Cannot execute the build target");
    });
    ...
    it("throws if app building fails", async () => {
      ...
      await expect(
        ftpDeploy(mockFtpEngine, builderContextMock, deployOptions)
      ).rejects.toThrowError("❌ Error while building the app.");
    });
    ...
    it("throws if there is no outputPath", async () => {
      ...
      await expect(
        ...
        `Cannot read the output path option of the Angular project '${builderContextMock.target?.project}' in angular.json`
      );
    });
  });
});

const initMockContext = (
  buildResult: boolean,
  target: Target = {
    project: "mockProject",
    target: "build",
  ...
  builderContextMock = {
    ...
    getProjectMetadata: (_: string | Target) =>
